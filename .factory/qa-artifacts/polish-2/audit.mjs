...
const output = resolve(process.argv[3] || '.factory/qa-artifacts/polish-2/local');
...
const demoBounds = {};
for (const selector of ['.run-status', '.stamp.fail', '.finding.fail>div>p', '.finding.fail summary']) {
  demoBounds[selector] = await page.locator(selector).evaluate((element) => element.getBoundingClientRect().toJSON());
}
await page.screenshot({ path: `${output}/demo-mobile-first-screen.png`, fullPage: false });
const storageOnArrival = await page.evaluate(async () => ({
...
await page.screenshot({ path: `${output}/demo-mobile-repair.png`, fullPage: true });
...
report.demo = {
  firstScreenBounds: demoBounds,
  completedReportFits: Object.values(demoBounds).every((box) => box.bottom <= 844),
...
for (const route of ['/', '/demo', '/privacy', '/terms', '/missing']) {
  ...
  report.routes[route] = await routePage.evaluate(({ status, routeErrors, violations }) => ({
    status,
  ...
  }), { status: response?.status(), routeErrors, violations: routeAxe.violations.map(({ id, impact }) => ({ id, impact })) });
